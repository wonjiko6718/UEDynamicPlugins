...
public:	
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Suspension") float SpringMaxExtension; // 최대 서스펜션 높이
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Suspension") float SpringMinExtension; // 최소 서스펜션 높이
...
public:
	...
	//Calc Functions
	UFUNCTION(BlueprintCallable) void CalcSuspensionForce(int WheelIdx); // 서스펜션 힘 연산(위로 미는 방향)
	UFUNCTION(BlueprintCallable) void CalcBodyPosture(); // 차체 자세 연산
